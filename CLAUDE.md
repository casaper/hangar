...
its three ports, so a session knows which clone it is without being told. It is generated by
`claudeLocalMdContent` and holds only what is true of that SESSION rather than of the fleet — this
file is already in its context, so nothing here is repeated there: the identity, the `SYNC PAUSE`
protocol from the receiving end, that `tmp/` is shared out from under it, and that
`orch-util sync <its own index>` would stash the tree it is working in. That file is excluded
