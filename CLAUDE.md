...
`.envrc.hangar`, `.nvmrc`, `.editorconfig`, `hangar.config.example.yaml`, `hangar.schema.json`,
`.gitignore` and `.claude/**`. Never the application, and never the contents of the two shared
directories it now also holds — `plans/` and `tmp/` are gitignored, and only their `.gitkeep`
files are tracked.

**Three things that used to be tracked here are not, and the rule behind all three is the same:
a tracked file that a `hangar` command rewrites is a merge conflict on every pull.** The two
shell helpers (`clone-colours.sh`, `clone-terminal.sh`) are regenerated by `hangar colours sync`
from `app/src/palette.ts`, so a committed copy is wrong for every hangar but the one that
committed it. `colour-assignments.json` moved to `.hangar/colour-assignments.json`, and that one
is **operator INPUT that nothing regenerates** — `hangar colours change` is the only writer — so
it is read from either path permanently and `hangar doctor --fix` moves a legacy one forward byte
for byte.
