...
`add-clone`, `install`, `remove-clone`, `exec`, `servers start`, `servers kill`, `colours change`,
`ports pin`, `ports unpin` and `doctor --fix`.
...
it is the one the command is running inside; `reload` restarts each idle shell and brings Claude
