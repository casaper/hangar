# Per-clone iTerm2 tab colour for the dvb_gn fleet, whenever the shell's PWD is inside any
# clone_NN directory (including any subdirectory).
#
# This file is hand-maintained, not generated: it holds no per-clone data at all. The hue
# table it needs lives in `clone-colours.sh`, which IS generated by `orch-util colours sync`, so
# adding or removing a clone never touches this file.
#
...
_dvb_iterm_chpwd() {
    ...
    if [[ "$PWD" == "$DVB_FLEET_ROOT"/* ]]; then
        ...
        first="${rest%%/*}"
        # Two or more digits: the fleet is not fixed at three clones and must not break
        # at clone_10, which is what the old clone_0[0-9] pattern would have done.
        [[ "$first" == clone_<->  ]] && { clone="$first"; rgb="$(dvb_clone_rgb "$clone")"; }
    fi
