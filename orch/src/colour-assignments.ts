 ...
 *
 * `orch-util colours change 4 red` has to persist SOMETHING, because the index formula in
 ...
 * reported by `orch-util doctor` -- silently reverting to the formula while looking edited is
 * exactly the failure this fleet keeps running into.
 */
const DOC_KEY = '_';

const DOC =
  'Written by `orch-util colours change <clone> <colour>`. Keys are clone indices; values are ' +
  'palette names from orch/src/palette.ts. A clone that is not listed takes PALETTE[(N-1) % length].';
