...
same name, for cherry-picking). Their working trees are off limits for writes.`;
  ...
  return [
    ...
    '',
    'The user can sync this clone while you are working, and `orch-util sync` has no way to message',
    ...
    '',
    '## `orch-util` is on your PATH — read with it, do not sync with it',
    '',
    'Reading is free and often the right move: `orch-util list`, `orch-util ports`, and',
    '`orch-util status <N>` to see what a sibling is up to before you read its tree.',
    '',
    `But \`orch-util sync ${String(clone.index)}\` — this clone — starts with a`,
    ...
    'the pause message above into this very session. `sync`, `open` and `remove-clone` are the',
    "user's to run, from the fleet root.",
    '',
    'This file is local to this clone, generated by `orch-util`, and excluded via',
    '`.git/info/exclude` — it never commits and never travels to a sibling. Do not hand-edit it:',
    '`orch-util doctor --fix` rewrites it whenever it differs from what the generator produces. For',
