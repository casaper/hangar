...
/**
 * `orch-util vscode sync` -- one VS Code setup across the fleet, with the per-clone paths
 * still per clone.
 *
...
export const vscodeSync = (opts: VscodeSyncOptions): void => {
  ...
  if (blocked) {
    throw new CliError(
      ...
      'A clone-specific setting is missing from SETTINGS_ROOT_KEYS in orch/src/vscode.ts.\n' +
