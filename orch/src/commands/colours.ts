...
/**
 * `orch-util colours change <clone> <colour>` -- give one clone a hue of your choosing.
 *
...
export const coloursChange = (ref: string, colour: string, opts: ColoursChangeOptions): void => {
  ...
  if (settings === undefined) {
    warn(
      `no readable .claude/settings.local.json — run \`orch-util doctor ${String(clone.index)} --fix\``,
    );
  } else {
...
/**
 * `orch-util colours list` -- the whole palette, painted, with who has what.
 *
