...
/**
 * `orch-util remove-clone` -- take a clone out of the fleet, and optionally delete it.
 *
...
export const removeClone = (ref: string, opts: RemoveCloneOptions): void => {
  ...
  note(`Index ${clone.index} is now free; the next \`orch-util add-clone\` will reuse it.`);
};
