 ...
 * `orch-util` behaves identically when invoked from a clone, from a subdirectory, or through a
 * shell alias. `DVB_FLEET_ROOT` still overrides it, matching the shell helpers.
 */
export const fleetRoot = resolve(process.env['DVB_FLEET_ROOT'] ?? dirname(import.meta.dirname));
