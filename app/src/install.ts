...
/**
 * Spawn one step, under fnm when it pins a Node version.
 *
 * Shell state does not survive between spawns, so the version is applied per command rather
 * than by sourcing anything -- and only when the step asked for it. Without fnm on PATH the
 * command still runs, on whatever Node is there, because a missing version manager is a worse
 * reason to refuse an install than a version mismatch is.
 */
const runStep = (
  item: PlannedStep,
  cmd: string,
  ...
  const res =
    version !== undefined && run('fnm', ['--version']).ok
      ? run('fnm', ['exec', `--using=${version}`, '--', cmd, ...args], {
          cwd: item.cwd,
          inherit: true,
        })
      : run(cmd, args, { cwd: item.cwd, inherit: true });
