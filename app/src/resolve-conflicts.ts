 ...
 *
 * **How the run itself works is `claude-headless.ts`** -- the streaming, the heartbeat, the
 * operator's line into a run already under way, and every measurement behind them. That module
 * was extracted from this file when a second command needed the same mechanism; what stayed here
 * is the half that is about conflicts, which is the prompt and the verification.
 */
...
export const resolveWithClaude = async (
  ...
  const run = await runHeadlessClaude({
    repo,
    prompt: prompt(operation, target, files),
  ...
  if (!run.ok) return { resolved: false, reason: run.reason };
  if (run.finalText !== '') for (const line of run.finalText.split('\n')) note(line);
