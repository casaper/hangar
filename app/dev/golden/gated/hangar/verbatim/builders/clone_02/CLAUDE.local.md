# This clone: clone_02 (yellow)

You are working in **`clone_02`**, one of the sibling clones under `~/code/dvb_gn/`.
...
`~/.claude/dvb_gn-clone-statusline.sh` (it derives the hue from this directory) plus the theme
`~/.claude/themes/dvb_gn-clone-02-yellow.json`, both selected in this clone's untracked
...
|                        |                                     |
| ---------------------- | ----------------------------------- |
| Root                   | `/Users/someone/code/dvb_gn/clone_02` |
| Colour                 | yellow                              |
| Angular dev server     | 4300                                |
| Storybook              | 6106                                |
| Playwright HTML report | 9423                                |
...
Every `tmp/<name>` entry is a symlink into `~/code/dvb_gn/tmp/`, and each cached Jira
