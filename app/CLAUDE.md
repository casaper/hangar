...
`cli` `fleet` `modes` `exec` `servers` `test`. That list is documented and deliberately **not** enforced — a
...
| commands              | `commands/*.ts`, one per command: `sync`, `doctor`, `tmp`, `jira`, `setup`, `open`, `checkout-default`, `vscode`, `plans`, `add-clone`, `resume`, `colours`, `status`, `remove-clone`, `teach-rg`, `config`, `ports`, `list`, `install`, `claude`, `browse`, `close`, `reload`, `pr`, `mcp`, `exec`, `edit`, `servers`                                                                                                                                                                                                                                                                                                          |
...
| shared                | `dedupe.ts`, `pr-cache.ts`, `pr-description.ts`, `claude-sessions.ts`, `resolve-conflicts.ts`, `claude-headless.ts`, `procs.ts`, `plans.ts`, `environment.ts`, `install.ts`, `secrets.ts`, `tui.ts`, `palette.ts`, `tmp.ts`, `sessions.ts`, `adopt.ts`, `ui.ts`, `hangar.ts`, `user-paths.ts`, `template.ts`, `exec.ts`                                                                                                                                                                                                                                                                                                         |
